...
const PanelDisplay: React.FC<PanelDisplayProps> = ({ panel, isMuted }) => {
  ...
  const stingerAudioRef = useRef<HTMLAudioElement>(null);
  ...
  useEffect(() => {
    ...
    // Play narrative, SFX, and stinger simultaneously for an immersive effect.
    playAudio(narrativeAudioRef.current);
    playAudio(sfxAudioRef.current);
    playAudio(stingerAudioRef.current);
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      {panel.stingerAudio && (
        <audio ref={stingerAudioRef} src={panel.stingerAudio} muted={isMuted} preload="auto" />
      )}
    </div>
  );
};
